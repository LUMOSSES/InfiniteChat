...
Write-Host "`nAll 7 services launched. Waiting for startup..."
...
foreach ($svc in $jars.Keys) {
    ...
    if (Test-Path $log) {
        ...
        $nacosOK = Select-String -Path $log -Pattern "register finished" -SimpleMatch -Quiet
        Write-Host "$svc : Started=$started, Nacos=$nacosOK"
    } else {
